...
--with cte 
--as
--( select * , DENSE_RANK()over(order by E_imdbRating desc)as DR from evaluation)
--select * from cte where DR<=8
...
declare c2 cursor
for 
   select p.ProductionCompany ,m.Title, m.Budget
   from PC p, Movie m
   update Movie 
   set Budget= Budget+ 50000
   where Budget < 50000
declare @co nvarchar(50) ,@name nvarchar(100), @b int 
open c2
fetch c2 into @co, @name, @b
while 
@@FETCH_STATUS=0
begin 
   select @co, @name, @b
